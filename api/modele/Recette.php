...
include("database.php");
    ...
    class Recette extends Database{
        ...
        public function createRecette($recette){
                    ...
                    $categorie_recette = htmlspecialchars($recette['categorie']);
                    ...
                    $liste_ingredient = htmlspecialchars($recette['liste_ingredients']);
                    $liste_ustensile = htmlspecialchars($recette['liste_ustensiles']);
                        ...
                        $upload = $_SERVER['DOCUMENT_ROOT']."/dev_web/site/images/".$time_name;
                            //déplacer un fichier téléchargé
                            move_uploaded_file($_FILES['image']['tmp_name'], $upload);

        
                    
                    $sql = "INSERT INTO Recettes (nom_recette, duree_recette, image_prix, image_difficulte, categorie_recette, image, recette_texte, ID_user, nombre_personne, liste_ingredients, liste_ustensiles) VALUES (:nom_recette, :duree_recette, :image_prix, :image_difficulte, :categorie_recette, :image, :recette_texte, :ID_user, :nombre_personne, :liste_ingredient, :liste_ustensile);";
                    ...
                    $query->bindValue(':image_prix', NULL, PDO::PARAM_STR);
                    $query->bindValue(':image_difficulte', NULL, PDO::PARAM_STR);
                    $query->bindValue(':categorie_recette', $categorie_recette, PDO::PARAM_STR);
                    $query->bindValue(':image', $time_name, PDO::PARAM_STR);
        ...
        public function GetCategorie($data){
            $sql="SELECT * FROM recettes WHERE categorie_recette=:categorie";
            $query=$this->pdo->prepare($sql);
            $query->execute(['categorie'=> $data]);
            $result = $query->fetchAll();
            return $result;
        }
     } 
